...
export const CurrencyDropdown = () =>{
  ...
  function handleOptionChange(option) {
    if (selectedCurrencies.find((c) => c.id === option.id)) {
      setSelectedCurrencies(selectedCurrencies.filter((c) => c.id !== option.id))
    } else {
  ...
  return (
    <Listbox value={selectedCurrencies} onChange={setSelectedCurrencies} multiple>
      <Listbox.Button>{selectedCurrencies.map((c) => c.name).join(', ')}</Listbox.Button>
      <Listbox.Options>
